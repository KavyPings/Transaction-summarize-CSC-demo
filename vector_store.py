...
# This module provides local sentence-transformer embeddings (all-MiniLM-L6-v2,
# 384-dim, free/CPU) and Pinecone serverless vector storage for evidence
# retrieval (PDFs / XLSXs).  It is disabled because at current scale the
# Pinecone query latency outweighs the benefit.
# See README § RAG Pipeline for re-enable instructions.
# ────────────────────────────────────────────────────────────────────────────
...
#     """Return the most relevant evidence chunks for a question (evidence type only)."""
#     index = _get_index()
#     results = index.query(

...
public class PortalApiTransactionDataService : ITransactionDataService
{
    private readonly IConfiguration _config;
    private readonly HttpClient _http;

    public PortalApiTransactionDataService(IConfiguration config, IHttpClientFactory httpFactory)
    {
        _config = config;
    ...
    public Task<IEnumerable<TransactionSummary>> ListTransactionsAsync()
        => throw new NotImplementedException(
            "PortalApiTransactionDataService is not yet implemented. " +
            "Set DataSource: \"LocalFiles\" in appsettings.json to use local files.");

    public Task<JsonDocument?> LoadTransactionAsync(string id)
        => throw new NotImplementedException("PortalApiTransactionDataService is not yet implemented.");
    ...
    public string BuildContextJson(JsonDocument doc)
        => throw new NotImplementedException("PortalApiTransactionDataService.BuildContextJson is not yet implemented.");
}
